...
def show(a, batches):
    ...
    if a.get("palette_hint"):
        ...
        print()
    print("  DO THIS")
    print(f"    1.  goxel {rel(manifest.guide_path(a))}")
    print(f"    2.  Build inside the magenta brackets. The cyan figure beside the")
    print(f"        box is 1.75 m tall; the cyan floor lines are 1 m apart.")
    print(f"        Guide colours are stripped automatically — leave them alone.")
    print(f"    3.  Save as:  {rel(manifest.src_path(a))}")
    print(f"    4.  python3 tools/build.py {a['name']}")
    print(f"    5.  Open the scale lab and look at it at the real camera angle:")
    print(f"        godot --path game res://scenes/lab/scale_lab.tscn")
    print(BAR)
